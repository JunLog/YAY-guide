...
metadata则是一些关于该对象的元数据，其中主要有`name`、`namespcae`、`labels`、`annotations`。其中， `name` 需要在 `namespace` 下唯一，成为这个对象的唯一标识。`label`和`annotations`分别是这个对象的一些标签和一些注解，前者用于筛选，后者主要用来标注提示性的信息。
